...
- `run_cli`: run one raw Bash command line inside the export root; prefer this over the higher-level search tools when you want direct full-text `rg` search behavior
...
- For broad or advanced full-text search, prefer `run_cli` with `rg` over `search_exports`.
